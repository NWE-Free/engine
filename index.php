...
try {
    ...
    if (isset($_SESSION['profiler']) && $_SESSION["profiler"] == "on") {
        ...
        $profilerInfo .= "<div class='pColA'>Query Time:</div><div class='pColB'>" . sprintf("%.0f ms",
                1000 * $totDbTime) . "</div>";
        echo "<style>";
        echo ".profilerDiv {position: fixed; top: 0px; width: 300px; color: black; background-color: white; border: solid 1px black; padding: 2px; margin: 5px; height: 20px; overflow: hidden; font-size: 12px; cursor: pointer; }";
        echo ".profilerCollaps { height: 20px; overflow: hidden; width: 300px; }";
